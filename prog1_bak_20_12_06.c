...
interrupt [TIM2_OVF] void timer2_ovf_isr(void)
        ...
        if (key_1==0)
                ...
                if (key_mode_press==0)
                        ...
                        if (pik_en==0)
                                {
                                key_mode_press_switch=1;pik_en=1;
                                drebezg=0;
                                }
                        }
                key_mode_press=1;
...
char ed,des,sot,tis,count_filter,count_filter1,i;
long filter_value;
...
long start_time,start_time_mode;
char n_filter;
...
while (1)
        ...
        for (count_filter=0;count_filter<9;count_filter++)
                {
                adc_buf=read_adc();
                filter_buf[count_filter]=adc_buf;
                }
       
        ed=0;des=0;filter_min=filter_buf[0];filter_max=filter_buf[0];
        for (i=0;i<9;i++) 
                {
                if (filter_buf[i]!=65535)
                        {
                        if (filter_min<filter_buf[i]){ed=i;filter_min=filter_buf[i];}
                        if (filter_max>filter_buf[i]){des=i;filter_max=filter_buf[i];}
                        }
                }
        filter_buf[ed]=65535;filter_buf[des]=65535;
        ed=0;des=0;filter_min=filter_buf[0];filter_max=filter_buf[0];
        for (i=0;i<9;i++) 
                {
                if (filter_buf[i]!=65535)
        ...
        if (++count_filter1>100) count_filter1=0;

        for (i=0;i<9;i++) 
                {
                if (filter_buf[i]!=65535)
                        {
                        filter_buf1[count_filter1]=filter_buf[i];
                        }
                }
        ...
        for (i=0;i<100;i++) 
                {
                filter_value=filter_value+filter_buf1[i];
                }
        filter_value=filter_value/100;
        adc_buf=filter_value;
        ...
        if (mode==1)//уставка №1
                {
                set_led_on(0,0,1,1,0,0,1,0);
                set_led_off(0,0,0,1,0,0,1,0);
