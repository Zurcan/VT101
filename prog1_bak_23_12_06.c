...
const flash signed int MAX_MIN[16,2]={{0,0},{-1000,1000},{-100,100},{-50,50},{-5000,5000},{-100,100},
...
while (1)
        ...
        for (j=0;j<9;j++)
                ...
                for (k=0;k<9;k++)
                        {
                        if (buf[j]<buf[k]) rang[j]--;
                        if (buf[j]>buf[k]) rang[j]++;
                        }
                }
        k=buf_end;

        xx=(6-time_filter)*0.002;
        xx=xx*2000;
        yy=1-((6-time_filter)*0.002);
        yy=yy*x;
        x=xx+yy;

      //  x=(float)((float)(time_filter*0,002)*buf[k])+(float)((1-(time_filter*0,002)*x));
        adc_buf=x+0.5;
